...
function post_to_third_party( $entry, $form ) {
    ...
    $post_url = 'http://crm.yesfs.com.au/api/post_lead/';
    $salary = floatval(rgar( $entry, '7' )) * 26;
    $body = array(
        'name' => rgar( $entry, '3' ), 
        'phone' => rgar( $entry, '8' ), 
        'email' => rgar( $entry, '5' ),
        'employment' => rgar( $entry, '6' ),    
        'salary' => $salary,
        'key' =>  'Ptw9hCl5bdWwg5oT8o5y'
    );
    ...
    $_SESSION['crm_id'] = $response[1];       
}  */
/*--------------------------------------------- FULL FORM ------------------------------------------------
------------------------------------------------------------------------------------------------------------*/

/* add_action( 'gform_after_submission_2', 'post_to_third_party2', 10, 2 );
function post_to_third_party2( $entry, $form ) {
    if (!session_id()) session_start();
    $key = $_SESSION['crm_id'];
    if ($key) {
        $post_url = "http://crm.yesfs.com.au/webforms/factfind/$key/submit/";
        $body = array(
            'promocode' => rgar( $entry, '3' ), 
            'loan_amount' => rgar( $entry, '4' ), 
            'name' => rgar( $entry, '5' ),
            'address' => rgar( $entry, '6' ),
            'state' => rgar( $entry, '50' ),
            'phone' => rgar( $entry, '49' ),
            'email' => rgar( $entry, '9' ),
            ...
            'family' => rgar( $entry, '13' ),
            'gender' => rgar( $entry, '14' ),
            'smoker' => rgar( $entry, '15' ),
            'children' => rgar( $entry, '16' ),
            'children_age' => rgar( $entry, '17' ),
            'occupation' => rgar( $entry, '19' ),
            'occupation_duties' => rgar( $entry, '20' ),
            'job_duration' => rgar( $entry, '21' ),
            'salary' => rgar( $entry, '22' ),
            'super_balance' => rgar( $entry, '23' ),
            'centerlink_income_type' => rgar( $entry, '24' ),
            'centerlink_income' => rgar( $entry, '25' ),
            'rent' => rgar( $entry, '27' ),
            'rent_amount' => rgar( $entry, '28' ),
            'max_repayment_weekly' => rgar( $entry, '29' ),
            'home_loan_amount' => rgar( $entry, '30' ),
            'credit_card_amount' => rgar( $entry, '31' ),
            'payday_loan_amount' => rgar( $entry, '32' ),
            'personal_loan_amount' => rgar( $entry, '33' ),
            'total_other_depts' => rgar( $entry, '34' ),
            'height' => rgar( $entry, '36' ),
            'weight' => rgar( $entry, '37' ),
            'medicaton' => rgar( $entry, '38' ),
            'medicaton_info' => rgar( $entry, '39' ),
            'medical_conditions' => rgar( $entry, '40' ),
            'medical_conditions_info' => rgar( $entry, '41' ),
            'doctor_tests' => rgar( $entry, '42' )
            'doctor_test_info' => rgar( $entry, '43' )
            'contact_time' => rgar( $entry, '45' )
            'contact_method' => rgar( $entry, '46' )
            'partner' => rgar( $entry, '47' )
